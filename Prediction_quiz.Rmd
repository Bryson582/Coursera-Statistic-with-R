...
model.full1 <- lm(log(price) ~ Overall.Qual + log(Garage.Area + 1) +
                  log(Total.Bsmt.SF + 1) + Garage.Cars + log(area) +
                  Full.Bath + Half.Bath +
                  Bedroom.AbvGr + Year.Built + log(X1st.Flr.SF) +
                  log(X2nd.Flr.SF + 1) +
...
# Model selection using AIC
model.AIC1 <- stepAIC(model.full1, k = 2)
demo_100 <- predict(model.AIC1)
demo_101 <- data.frame(demo_100)
exp(mean(demo_100))
```
...
# Extract Predictions
predict.full3 <- exp(predict(model.AIC, ames_test))
...
# Extract Residuals
resid.full1 <- ames_train$price - predict.full1
...
# type your code for Question 2 here, and Knit
predict.full2 <- exp(predict(model.AIC, ames_train))

# Extract Residuals
resid.full2 <- ames_train$price - predict.full2
...
model.fulla <- lm(log(price) ~ Overall.Qual + log(Garage.Area + 1) +
                  log(Total.Bsmt.SF + 1) + Garage.Cars + log(area) +
                  Full.Bath + Half.Bath +
                  Bedroom.AbvGr + Year.Built + log(X1st.Flr.SF) +
                  log(X2nd.Flr.SF + 1) +
...
model.fullb <- lm(log(price) ~ Overall.Qual +
                  log(Total.Bsmt.SF + 1) + Garage.Cars + log(area) +
                  Bedroom.AbvGr + Year.Built + log(X1st.Flr.SF) +
...
BIC(model.fullb)
summary(model.fullb)
predict.fulls <- exp(predict(model.fullb, ames_test, interval = "prediction"))

# Calculate proportion of observations that fall within prediction intervals
 coverage.prob.fulls <- mean(ames_test$price > predict.fulls[,"lwr"] &
                             ames_test$price < predict.fulls[,"upr"])
 coverage.prob.fulls
```
