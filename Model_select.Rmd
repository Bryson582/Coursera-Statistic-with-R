...
library(MASS)
demo_772 <- lm ( log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope , data = ames_train )
demo_723 <- step(demo_772)
summary(demo_723)
```
...
demo_724 <- lm(log(price) ~ Bedroom.AbvGr,data =ames_train )
demo_724
demo_725 <- lm(log(price) ~ Bedroom.AbvGr + log(area) , data = ames_train)
demo_725
demo_726 <- step(demo_725)
```
...
demo_773 <- lm(log(price) ~ log(area),data = ames_train)
demo_774 <- data.frame(demo_773$residuals,ames_train$Neighborhood)
demo_774 %>%
group_by(ames_train.Neighborhood) %>%
  summarise(avg = mean(demo_773.residuals))
qqnorm(demo_773$residuals)
qqline(demo_773$residuals)
```
...
demo_774 %>%
group_by(ames_train.Neighborhood) %>%
  summarise(n = mean(demo_773.residuals) **2)
```
...
demo_775 <- lm(log(price) ~ Overall.Qual + Bsmt.Qual + Garage.Qual ,data = ames_train)
summary(demo_775)
```
...
demo_776 <- lm(log(price) ~ Overall.Cond + Overall.Qual , data = ames_train )
demo_777 <- data.frame( a = ames_train$MS.SubClass, b = predict(demo_776))
demo_777 %>%
  group_by(demo_777$a) %>%
  summarise(n = median(b))
...
demo_776 <- lm(log(price) ~ Overall.Cond + Overall.Qual , data = ames_train )
demo_777 <- hatvalues(demo_776,do.coef = FALSE)
demo_778 <- data.frame(demo_777)
demo_778[125,1]
demo_778[268,1]
demo_778[640,1]
demo_778[832,1]
...
demo_779 <- subset(ames_train,ames_train$Sale.Condition=="Normal",select = c(1:81))
```
...
demo_773 <- lm(log(price) ~ log(area),data = ames_train)
summary(demo_773)
demo_774 <- lm(log(price) ~ log(area),data = demo_779)
summary(demo_774)
```
